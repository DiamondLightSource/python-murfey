"""
LIF reader written based on readlif, but with added features
Source: https://github.com/nimne/readlif/blob/master/readlif/reader.py
"""
    ...
    def get_plane(self, display_dims=None, c=0, requested_dims=None):
        ...
        for i in self.dims_n.keys():
            if (requested_dims[i] + 1) > self.dims_n.get(i, 0):
                raise ValueError(
                    f"Requested frame in dimension {str(i)} " f"doesn't exist"
                )
        ...
        with open(self.filename, "rb") as image:
            ...
            precalc_dim_prod = tuple(
                [
                    reduce(lambda x, y: x * y, dim_len[:i])
                    for i in key_idx
                    if len(dim_len[:i]) > 0
                ]
            )
        ...
        for item in children:
            ...
            if is_image:
                ...
                data_dict = {
                    ...
                    "path": str(path + "/"),
                    "name": "/".join(
                        (str(path + "/") + item.attrib["Name"]).split("/")[1:]
                    ),
                    "channels": n_channels,
