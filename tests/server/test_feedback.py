...
def test_feedback_callback(
    ...
    mocker.patch("murfey.server.feedback.get_security_config", return_value=MagicMock())
    mocker.patch("murfey.server.feedback.url", return_value=MagicMock())
    mocker.patch("murfey.server.feedback.create_engine", return_value=MagicMock())
    mock_murfey_db = MagicMock()
    mock_sql_session = mocker.patch("murfey.server.feedback.Session")
    mock_sql_session.return_value.__enter__.return_value = mock_murfey_db
