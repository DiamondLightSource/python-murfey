...
def test_run_with_db(
    ...
    assert len(milling_step_ispyb_rows) == len(milling_steps)
    ...
    assert len(milling_step_murfey_rows) == len(milling_steps)
...
def test_run_fails(
    ...
    # Mock the site info dictionary accordingly
    project_name = visit_name if has_project_name else None
    site_name = "Lamella" if has_site_name else None
    site_num = 1 if has_site_number else None
    message = {
        "register": "fib.register_milling_progress",
        "session_id": session_id,
        "site_info": {
            ...
            "stage_info": {
                "preparation_site": {
                    "x": -0.0031091224743875403 if has_slot_number else None,
                    "y": 0.00420867925495798,
                    "z": 0.0323644854106331,
                    "rotation": 285.003247202109,
                    "tilt_alpha": 25.9996646026832,
